...
const aboutPersonal: AboutPersonal = {
  ...
  "siteTimeline": [
    ...
    },
    {
      "date": "2026.8.9",
      "content": [
        "重命名仓库，并修复base 路径，开源本仓库初始化主题仓库",
        "后台新增「站点信息」编辑器（标题/标语/创建时间），「首页签名」编辑器（打字机内容可编辑）"
      ]
    },
    {
      "date": "2026.8.12",
    ...
    },
    {
      "date": "2026.8.18",
      "content": [
        "新增 8 个自定义样式按钮：下划线、模糊、彩虹、拼音、脚注、引用卡片、音乐卡片、GitHub 卡片",
        "新增对应插入动作，脚注自动选取未占用编号并在文末补全定义",
        "隐藏全局回顶按钮，回顶统一由收拢式浮动按钮提供",
        "FAB 主按钮图标由三点改为加号，展开时旋转 135°",
        "新增 MobileFab 移动端圆钮，带阅读进度环，可展开菜单、目录、返回上一页、评论、回顶",
        "移除顶栏汉堡按钮显式入口，保留 DOM 供 FAB 代理点击",
        "小于 768px 时隐藏文章页旧浮动按钮簇",
        "侧边抽屉删除与底部导航重复的首页、归档、关于、工具栏及提示语",
        "搜索、主题、指南、邮箱、字体等快捷操作在所有移动端宽度显示，并重设计为图标按钮",
        "预览窗新增自定义语法预处理，支持 :::quote、提示框、推导容器、音乐卡片、GitHub 卡片",
        "引用卡片预览与正文同款，right 映射为落款样式",
        "新增脚注引用与文末定义列表渲染",
    ...
    },
    {
      "date": "2026.9.22",
      "content": [
        "视觉：统一圆角/表面/阴影/层级设计令牌与 767px 移动端断点，顶栏改为 grid 三列布局，文章卡片标签并入分类色体系",
    ...
    },
    {
      "date": "2026.9.25",
      "content": [
        "归档：文章列表统一为“日期 + 标题 + 分类”的单行式，月份标题改用细分隔线；悬停或键盘聚焦时平滑展开摘要，并加入标题位移、指示条与弹性动效",
        "交互：分类导航由药丸徽章改为清单行，二级分类切换改为交叉过渡与容器高度平滑生长；归档新增“全部展开/收起”并记忆本地折叠状态",
        "视觉：恢复工具页卡片的悬停倾斜翻面效果，保留触屏、键盘聚焦与减少动画适配",
        "修复：分类标签配色变量改为在标签元素上解析，恢复文章卡片、归档列表与 404 推荐标签在明暗主题下的分类色",
        "分类：高数、线代、复变函数统一归入数学，数学归入学习笔记",
        "归档：支持学习笔记 → 数学 → 具体学科的三级分类导航"
      ]
    }
  ],
  "musicTracks": [
